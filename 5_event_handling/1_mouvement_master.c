...
static int	validation(t_master *master, t_xy next_pos)
{
	...
	if ((next_pos.x < 0 || next_pos.y < 0 || next_pos.x >= master->map.map_size_x || next_pos.y >= master->map.map_size_y) || master->map.original_map[(int)next_pos.y][(int)next_pos.x] != '0')
		return (-1);
	return (0);
}
